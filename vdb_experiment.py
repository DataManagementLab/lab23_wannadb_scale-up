...
indicies_to_test = ["FLAT",
                    ...
                    "IVF_SQ8",
                    "GPU_IVF_FLAT"]
PATH = "D:\\UNI\\wannaDB\\datasets\\corona\\raw-documents"
...
def test_new_implementation():
    ...
    for index_type in indicies_to_test:
        ...
        generate_new_index(index_params)
        try:
            result_dict = compute_new_vdb_distances()
            time_without, distances_without = compute_embedding_distances_withoutVDB()
    ...
    for index_type, result in results.items():
        ...
        for attr_name, attr_time in result["vdb"].items():
            row = startrow
            times_worksheet.write(row, column, attr_name)
            row +=1
            times_worksheet.write(row, column, attr_time)
