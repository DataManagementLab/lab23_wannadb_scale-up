...
logging.basicConfig(filename='mein_log.log', level=logging.INFO)
...
@register_configurable_element
class RankingBasedMatcherVDB(BaseMatcher):
    """Matcher that displays a ranked list of nuggets to the user for feedback. Based on vector database"""

    identifier: str = "RankingBasedMatcherVDB"
    ...
    def __init__(
        ...
        :param find_additional_nuggets: optional function to add nuggets similar to a manually added and matched nugget
        """
        super(RankingBasedMatcherVDB, self).__init__()
        self._max_num_feedback: int = max_num_feedback
        self._len_ranked_list: int = len_ranked_list
        self._default_max_distance: float = max_distance
        self._max_distance: float = max_distance
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> "RankingBasedMatcherVDB":
        return cls(config["max_num_feedback"], config["len_ranked_list"], config["max_distance"],
                   config["num_random_docs"], config["sampling_mode"], config["adjust_threshold"],
                   Pipeline.from_config(config["nugget_pipeline"]))